...
<!--
	...
	function searchName() {
		...
		xmlhttp.onreadystatechange = function() {
			...
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200 && nameValue != '') {

				//var jsonObj = eval("(" + xmlhttp.responseText + ")");
				 var jsonObj = eval(xmlhttp.responseText);//将json字符串转化为json对象（可以看为一个数组）
				
				if(jsonObj.length>0){
					document.getElementById("content").style.display="block";
					for ( var i = 0; i < jsonObj.length; i++) {
						div.innerHTML += "<div style='z-index:10000000' onclick='fillNameValue(this)' onmouseover='changeBackground_over(this)' onmouseout='changeBackground_out(this)'>"
								+ jsonObj[i] + "</div>"
					}
				}
		...
		//3.open
		xmlhttp.open("GET",
				"${pageContext.request.contextPath}/servlet/searchBookAJAXServlet?name="
...
<div id="content"
	style="z-index:99999;background-color:white;width:128px; text-align:left;margin-left:945px;color:black;float:left;margin-top: -20px;display: none">
</div>